...
export default function Inbox() {
  ...
  // ✅ Get current user from token & register socket
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        const grouped = allMessages.reduce((acc, msg) => {
          const otherUser =
            msg.sender._id === currentUserId ? msg.recipient : msg.sender;
          ...
          {conversations.length === 0 ? (
            ...
            <div className="conversations-list">
              {conversations.map((conv) => {
                const isYouSender =
                  conv.messages[conv.messages.length - 1]?.sender?._id === currentUserId;
                return (
                  <div
                    key={conv.id}
